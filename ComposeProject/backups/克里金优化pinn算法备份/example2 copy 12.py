...
DYNAMIC_LOSS_RATIO_START = 10.0     # 🚀 动态策略起始比值：前半段训练的数据权重/物理权重比值
DYNAMIC_LOSS_RATIO_END = 0.1       # 🎯 动态策略结束比值：后半段训练结束时的比值
FIXED_LOSS_RATIO = 5.0              # 📌 固定策略比值：当不启用克里金时使用的固定比值
...
def main():
    ...
    if USE_DYNAMIC_STRATEGY:
        print(f"     ├─ 前半段训练: 数据权重/物理权重 = {INITIAL_LOSS_RATIO:.1f} (专注数据学习)")
        print(f"     ├─ 后半段训练: {INITIAL_LOSS_RATIO:.1f} → {FINAL_LOSS_RATIO:.1f} (配合克里金)")
        print(f"     └─ 克里金启用阶段: 后半段训练 ({TOTAL_EPOCHS//2}轮后)")
    else:
    ...
    print(f"\n🎯 动态训练策略:")
    if USE_DYNAMIC_STRATEGY:
        print(f"   📚 第一阶段 (0-{training_phase_transition_point}轮): 数据学习专注期")
        print(f"      └─ 损失权重比值: {INITIAL_LOSS_RATIO:.1f} (固定)")
        print(f"      └─ 克里金重采样: ❌ 禁用")
        print(f"   🧠 第二阶段 ({training_phase_transition_point}-{TOTAL_EPOCHS}轮): 克里金优化期")
        print(f"      └─ 损失权重比值: {INITIAL_LOSS_RATIO:.1f} → {FINAL_LOSS_RATIO:.1f} (线性递减)")
        print(f"      └─ 克里金重采样: ✅ 启用")
    else:
    ...
    while total_epochs_trained < TOTAL_EPOCHS:
        ...
        if USE_DYNAMIC_STRATEGY:
            ...
            if is_in_kriging_phase:
                ...
                new_loss_ratio = INITIAL_LOSS_RATIO - progress_in_second_phase * (INITIAL_LOSS_RATIO - FINAL_LOSS_RATIO)
            ...
            print(f"   - 训练阶段: {'第二阶段(克里金优化)' if is_in_kriging_phase else '第一阶段(数据学习)'}")
        ...
        if should_trigger_intervention:
            ...
            if kriging_enabled_this_cycle:  # [修改] 使用动态策略控制的克里金启用状态
                ...
                if USE_DYNAMIC_STRATEGY and total_epochs_trained < training_phase_transition_point:
                    ...
                    print("INFO: 配置点保持不变，当前处于数据学习专注期。")
