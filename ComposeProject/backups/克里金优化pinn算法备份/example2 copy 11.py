...
def main():
    ...
    # --- [新增] 损失权重比值配置 ---
    # 🔧 LOSS_RATIO: 数据损失权重 / 物理损失权重的比值
    # 
    # 📊 不同策略的克里金优化效果分析:
    # 
    # 🔬 物理主导策略 (loss_ratio < 1.0):
    # - 0.5: 物理权重2倍于数据权重，强调PDE一致性
    # - 0.8: 物理权重稍强，残差分布更平滑连续
    # - 优势: 有利于克里金空间插值，外推预测更可靠
    # - 适用: 物理方程高度可信、数据有噪声的场景
    #
    # ⚖️ 平衡策略 (loss_ratio = 1.0 ~ 5.0):
    # - 1.0: 完全平衡，物理和数据权重相等
    # - 3.0: 轻微偏向数据拟合，保持物理约束 
    ...
    # 🎯 数据主导策略 (loss_ratio > 5.0):
    # - 8.0+: 强调数据拟合，适合高质量数据
    # - 风险: 可能导致残差分布不均，影响克里金效果

    # 🔧 快速选择损失比值 (取消注释使用):
    # LOSS_RATIO = LOSS_RATIO = LOSS_RATIO_PHYSICS_DOMINANT_SUPER # 0.1 - 超级物理主导 (推荐试验!) 
    LOSS_RATIO = LOSS_RATIO_PHYSICS_DOMINANT # 0.5 - 物理主导 (推荐试验!)
    # LOSS_RATIO = LOSS_RATIO_PHYSICS_STRONG   # 0.8 - 物理优先  
    # LOSS_RATIO = LOSS_RATIO_EQUAL_BALANCE    # 1.0 - 完全平衡
    # LOSS_RATIO = LOSS_RATIO_CONSERVATIVE     # 3.0 - 强调物理约束
    # LOSS_RATIO = LOSS_RATIO_BALANCED         # 5.0 - 平衡策略 (当前默认)
    # LOSS_RATIO = LOSS_RATIO_AGGRESSIVE       # 8.0 - 偏重数据拟合
    # LOSS_RATIO = LOSS_RATIO_CURRENT_DEFAULT  # 10.0 - 原始默认值
    # LOSS_RATIO = LOSS_RATIO_TOO_HIGH         # 15.0 - 测试过度拟合的影响

    print("\n" + "="*60)
    print("🚀 开始执行：自适应PINN训练实验")
    print("="*60)
    print(f"📋 实验配置:")
    print(f"   - 损失权重比值: {LOSS_RATIO:.1f} (数据损失/物理损失)")
    print(f"     └─ Loss weights: [物理={1.0:.1f}, 数据={LOSS_RATIO:.1f}]")
    ...
    pinn = PINNModel(
        ...
        loss_ratio=LOSS_RATIO  # [新增] 传入配置的损失比值
    )
    kriging = GPUKriging()
    ...
    while total_epochs_trained < TOTAL_EPOCHS:
        ...
        if should_trigger_intervention:
            ...
            if ENABLE_KRIGING:
                ...
                print(f"      - 当前loss_ratio: {LOSS_RATIO:.1f} 的残差分布特征分析完成")
                ...
                if residual_cv < 0.5:
                    ...
                    print(f"      - ❌ 残差分布变异过大，建议降低loss_ratio改善物理约束")
    ...
    print(f"INFO: 基线PINN将使用与自适应PINN相同的 {len(full_training_data)} 个训练点")
    ...
    pinn_baseline = PINNModel(
        ...
        test_data=test_data,
        num_collocation_points=NUM_COLLOCATION_POINTS,
        loss_ratio=LOSS_RATIO  # [新增] 使用相同的损失比值确保公平对比
    )
