...
DYNAMIC_LOSS_RATIO_START = 0.1     # 🚀 动态策略起始比值：前半段训练的数据权重/物理权重比值
DYNAMIC_LOSS_RATIO_END = 10       # 🎯 动态策略结束比值：后半段训练结束时的比值
...
def main():
    ...
    if USE_DYNAMIC_STRATEGY:
        print(f"     ├─ 前半段训练: 数据权重/物理权重 = {INITIAL_LOSS_RATIO:.1f} (专注物理约束)")
        print(f"     ├─ 后半段训练: {INITIAL_LOSS_RATIO:.1f} → {FINAL_LOSS_RATIO:.1f} (指数增长，数据精化)")
        print(f"     ├─ 增长策略: 前期缓慢增长，让物理权重主导期有充足训练时间")
        print(f"     └─ 克里金策略: 仅前半段启用，避免在高数据权重下使用不均匀残差分布")
    else:
    ...
    # [新增] 动态策略相关变量
    training_phase_transition_point = TOTAL_EPOCHS // 2  # 训练阶段转换点
    kriging_enabled_this_cycle = False  # 当前周期是否启用克里金
    current_loss_ratio = INITIAL_LOSS_RATIO  # 当前使用的损失权重比值
    
    print(f"\n🎯 动态训练策略:")
    if USE_DYNAMIC_STRATEGY:
        print(f"   🔬 第一阶段 (0-{training_phase_transition_point}轮): 物理约束建立期")
        print(f"      ├─ 损失权重比值: {INITIAL_LOSS_RATIO:.1f} (物理主导，建立平滑残差分布)")
        print(f"      └─ 克里金重采样: ✅ 启用 (利用平滑残差分布优化配置点)")
        print(f"   📈 第二阶段 ({training_phase_transition_point}-{TOTAL_EPOCHS}轮): 数据精化期")
        print(f"      ├─ 损失权重比值: {INITIAL_LOSS_RATIO:.1f} → {FINAL_LOSS_RATIO:.1f} (指数增长)")
        print(f"      ├─ 增长特点: 前期缓慢增长，后期加速")
    ...
    while total_epochs_trained < TOTAL_EPOCHS:
        ...
        if USE_DYNAMIC_STRATEGY:
            ...
            if is_in_kriging_phase:
                # 第二阶段：禁用克里金（数据权重高导致残差分布不均匀，不利于克里金）
                kriging_enabled_this_cycle = False
            else:
                # 第一阶段：启用克里金（物理主导产生平滑残差分布，利于克里金）
                kriging_enabled_this_cycle = True
            
            if is_in_kriging_phase:
                # 第二阶段：指数增长损失权重比值（从物理主导逐步转向数据主导）
                ...
                # 使用指数增长策略，前半段缓慢增长，后半段加速增长
                # 公式：ratio = start * ((end/start)^(progress^2))
                # progress^2 让前期增长缓慢，后期加速，确保物理权重主导期有足够训练时间
                growth_factor = (FINAL_LOSS_RATIO / INITIAL_LOSS_RATIO) ** (progress_in_second_phase ** 2)
                new_loss_ratio = INITIAL_LOSS_RATIO * growth_factor
                
                # 确保不超过最终目标值
                new_loss_ratio = min(new_loss_ratio, FINAL_LOSS_RATIO)
                ...
                if total_epochs_trained == training_phase_transition_point or (total_epochs_trained < training_phase_transition_point + ADAPTIVE_CYCLE_EPOCHS and cycle_counter <= 1):
                    print(f"\n🔄 进入第二阶段: 数据精化期")
                    print(f"   📈 开始逐步增加数据权重")
                    print(f"   ❌ 克里金重采样已禁用（避免在高数据权重下使用不均匀残差分布）")
            ...
            print(f"   - 训练阶段: {'第二阶段(数据精化)' if is_in_kriging_phase else '第一阶段(物理约束建立)'}")
            ...
            if is_in_kriging_phase:
                ...
                if current_loss_ratio < 1.0:
                    ...
                    print(f"   - 📊 当前接近平衡状态，正在向数据主导转变")
                else:
                    print(f"   - 📈 当前数据权重占主导，克里金引导下精细拟合")
        ...
        if should_trigger_intervention:
            ...
            if kriging_enabled_this_cycle:  # [修改] 使用动态策略控制的克里金启用状态
                ...
                if USE_DYNAMIC_STRATEGY and total_epochs_trained >= training_phase_transition_point:
                    print("\nPHASE B: 克里金重采样在第二阶段禁用，避免高数据权重下的不均匀残差分布。")
                    print("INFO: 配置点保持不变，当前处于数据精化期。")
