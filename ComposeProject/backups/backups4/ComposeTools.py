...
class Mode1Fusion:
    ...
    def fuse_residual(pinn_pred: np.ndarray,
        ...
        """
        if uncertainty is not None and len(uncertainty) == len(pinn_pred):
            # --- 新增：基于不确定度的自适应权重计算 ---
            # 调用自适应权重策略计算动态权重
            dynamic_weights = Mode1Fusion.adaptive_weight_strategy(
                kriging_std=np.sqrt(np.maximum(uncertainty, 0)), # 策略函数需要标准差
                strategy='variance_based'
            )
            # 使用动态权重进行融合
            fused_pred = pinn_pred + dynamic_weights * kriging_residual
            return fused_pred, dynamic_weights
        else:
            # --- 保留：旧的静态权重融合逻辑 ---
            if uncertainty is not None:
                 warnings.warn("提供了不确定度，但长度与预测不匹配，回退到静态权重融合")
            fused_pred = pinn_pred + weight * kriging_residual
            return fused_pred, np.full_like(fused_pred, fill_value=weight)
    
    @staticmethod
    def adaptive_weight_strategy(residuals: np.ndarray=None,
        ...
        Args:
            residuals: 训练点上的残差 (当前未使用)
            kriging_std: 预测点上Kriging预测的标准差
            strategy: 策略名称. 当前只支持 'variance_based'

        Returns:
            每个预测点的权重数组
        """
        if strategy == 'variance_based' and kriging_std is not None:
            # 1. 将标准差转换为方差
            ...
            if max_var - min_var < EPSILON:
                # 如果所有方差都相同，则给予一个中性的恒定权重
                return np.full_like(kriging_variance, fill_value=0.5)
                
            normalized_variance = (kriging_variance - min_var) / (max_var - min_var)
            
            # 3. 计算权重：方差越大，权重越小 (1 - normalized_variance)
            # 权重范围在 [0, 1]
            weights = 1.0 - normalized_variance
            return weights
        else:
            # 默认或回退情况
            warnings.warn(f"不支持的自适应权重策略'{strategy}'或未提供标准差，返回默认权重0.5")
            return np.full_like(kriging_std, fill_value=0.5) if kriging_std is not None else np.array([0.5])
...
class CouplingWorkflow:
    ...
    def run_mode1_pipeline(self,
                          ...
                          residual_split_ratio: float = 2/3., # 新增：数据划分比例
                          **kwargs) -> Dict[str, Any]:
        """
        执行方案1: PINN -> 残差Kriging -> 融合
        Run Mode 1: PINN -> Residual Kriging -> Fusion

        Args:
            train_points: 初始训练数据点坐标 (N, D)
            train_values: 初始训练数据点真值 (N,)
            prediction_points: 需要进行最终预测的点坐标 (M, D)
            fusion_weight: [将被弃用] 静态融合权重
            dose_data: PINN所需的剂量数据
            residual_split_ratio: 用于PINN训练的数据比例 (剩余的用于残差计算)
            **kwargs: 其他传递给PINN训练的参数 (如 epochs)
        ...
        if self.config.verbose:
            print("\n" + "-"*20 + " 开始执行方案1 pipeline " + "-"*20)
            print(f"PINN训练集/残差计算集划分比例: {residual_split_ratio:.2f} / {1-residual_split_ratio:.2f}")

        # --- 1. 数据划分 (对应思路: "首先将已知点划分为两部分") ---
        if not (0 < residual_split_ratio < 1):
            raise ValueError("residual_split_ratio必须在(0, 1)之间")
        
        pinn_train_points, residual_points, pinn_train_values, residual_values = train_test_split(
            train_points, train_values, train_size=residual_split_ratio, random_state=self.config.random_seed
        )
        if self.config.verbose:
            print(f"数据划分完成: {len(pinn_train_points)}个点用于PINN训练, {len(residual_points)}个点用于残差计算。")

        # --- 2. 初始PINN训练 (对应思路: "多的部分用于训练PINN") ---
        print("\n--- 步骤 1/3: 在划分出的子集上训练初始PINN模型 ---")
        pinn_adapter = PINNAdapter(self.physical_params, self.config)
        pinn_adapter.fit_from_memory(
            train_points=pinn_train_points, 
            train_values=pinn_train_values, 
            dose_data=dose_data, 
            **kwargs
        )
        
        # --- 3. 计算PINN预测和残差 (对应思路: "少的部分用PINN预测后与在其上的真实值计算残差") ---
        print("\n--- 步骤 2/3: 计算PINN预测和残差 ---")
        # a) 在最终预测点上进行预测，作为基线
        pinn_predictions_final = pinn_adapter.predict(prediction_points)
        results['pinn_predictions'] = pinn_predictions_final
        if self.config.verbose:
            print(f"已生成 {len(pinn_predictions_final)} 个基线PINN预测。")

        # b) 在独立的残差点集上进行预测，用于计算残差
        pinn_predictions_on_residual_points = pinn_adapter.predict(residual_points)
        
        # c) 计算残差 (真实值 - 预测值)
        residuals = residual_values - pinn_predictions_on_residual_points
        if self.config.verbose:
            print(f"已在 {len(residual_points)} 个独立点上计算残差。")
            
        # --- 4. 对残差进行Kriging插值 (对应思路: "使用得到的残差来训练克里金插值") ---
        # 注意：residual_kriging方法内部会调用compute_residuals，但我们现在要绕过它
        # 我们将直接训练Kriging，然后用它来预测
        print("\n--- 步骤 3/3: 对残差进行Kriging插值并融合 ---")
        residual_kriging_module = self.mode1_tools['residual_kriging']
        
        # a) 直接用独立的残差数据训练Kriging
        print("   - 训练残差Kriging模型...")
        residual_kriging_module.kriging_adapter.fit(residual_points, residuals)

        # b) 预测全场残差和不确定度
        print("   - 预测全场残差及不确定度...")
        kriging_residual, kriging_uncertainty = residual_kriging_module.kriging_adapter.predict(
            prediction_points, return_std=True
        )
        results['kriging_residual'] = kriging_residual
        results['kriging_uncertainty'] = kriging_uncertainty
        if self.config.verbose:
            print(f"   - 已完成对 {len(kriging_residual)} 个点的残差Kriging插值。")

        # --- 5. 融合结果 (对应思路: "根据克里金预测残差的不确定度来确定...权重") ---
        print("   - 使用自适应权重融合结果...")
        final_predictions, dynamic_weights = self.mode1_tools['fusion'].fuse_residual(
            pinn_pred=pinn_predictions_final,
            kriging_residual=kriging_residual,
            uncertainty=kriging_uncertainty**2 # fuse_residual需要方差
        )
        ...
        if self.config.verbose:
            print("融合完成。")
            avg_weight = np.mean(dynamic_weights) if len(dynamic_weights) > 0 else 0
            std_weight = np.std(dynamic_weights) if len(dynamic_weights) > 0 else 0
            print(f"动态权重统计: 平均值={avg_weight:.3f}, 标准差={std_weight:.3f}")

        end_time = time.time()
        results['total_time'] = end_time - start_time
        print(f"\n方案1 pipeline 执行完毕，总耗时: {results['total_time']:.2f} 秒。")
        print("-" * 60)
